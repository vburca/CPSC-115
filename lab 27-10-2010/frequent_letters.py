...
for line in f:
    ...
    for c in alphabet:
        if has_ch(word, c):
            frequency[ord(c) - 97] = frequency[ord(c) - 97] + 1     
...
for i in range(len(frequency)):
    if max_freq < frequency[i]:
        ...
        maxc_freq = chr(i+97)
for i in range(len(frequency)):
    print frequency[i], " words use '", chr(i+97), "' ,"
print "and '", maxc_freq, "' is the most frequently-used letter."
